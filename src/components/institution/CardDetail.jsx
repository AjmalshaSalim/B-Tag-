...
function CardDetail() {
  ...
  const card = data.find((each) => each.slug === slug);

  // Handle case where card might not be found
  if (!card) {
  ...
  // Get the second image from the images field
...
//     const {slug}=useParams()
//     // const card=data.find((each) => each.id=== parseInt(id))
//     const card=data.find((each) => each.slug=== slug)
//     const paragraphs = card.description?.split('\n\n');
